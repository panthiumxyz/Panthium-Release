...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        if not os.path.exists("configs/default.json"):
            fo = open(fr"configs/default.json", "w")
            fo.close()
        ...
        def Create_config():
            ...
            if config_name != "":
                ...
                if not os.path.exists(fr"configs/{config_name}.json"):

                    fo = open(fr"configs/{config_name}.json", "w")
                    fo.close()

                    self.ConfigList.configure(values=os.listdir("configs"))
                    self.ConfigList.set(f"{config_name}.json")
        ...
        def Save_config():
                ...
                if config_name == "default.json":
                    ...
                    if os.path.exists(fr"configs/{config_name}"):
                        ...
                        fo = open(fr"configs/{config_name}", "w")
                        fo.write(json.dumps({
                            "FOV": self.FOV_Slider.get(),
                            "Smoothing": self.Smoothing_Slider.get(),
                            "Aimbot_Key": self.Aimbot_Key.get(),
                            "Aimbone": self.Aimbone.get(),
                            "Threshhold": self.Threshhold_Slider.get(),
                            "Bhop": self.BhopToggle.get(),
                            "Alt_Aimbot": self.alt_aimbone.get(),
                            "Triggerbot": self.Triggerbot_enabled.get(),
                            "Triggerbot_Key": self.triggerbot_key.get(),
                            "Alt_Aimbot_Key": self.alt_aimbone_select.get(),
                            "Triggerbot_Type": self.Trigger_type.get(),
        ...
        def Load_config():
            ...
            if os.path.exists(fr"configs/{config_name}"):
                with open(fr"configs/{config_name}", "r") as f:
                    data = json.load(f)  # Corrected method name

                    self.FOV_Slider.set(data["FOV"])
                    self.Smoothing_Slider.set(data["Smoothing"])
                    self.Aimbot_Key.set(data["Aimbot_Key"])
                    self.Aimbone.set(data["Aimbone"])
                    self.Threshhold_Slider.set(data["Threshhold"])
                    if data["Bhop"] != self.BhopToggle.get():
                        self.BhopToggle.toggle()
                    if data["Alt_Aimbot"] != self.alt_aimbone.get():
                        self.alt_aimbone.toggle()
                    if data["Triggerbot"] != self.Triggerbot_enabled.get():
                        self.Triggerbot_enabled.toggle()

                    if data["Accelerate"] != self.Accelerate.get():
                        self.Accelerate.toggle()
                    self.triggerbot_key.set(data["Triggerbot_Key"])
                    self.alt_aimbone_select.set(data["Alt_Aimbot_Key"])
                    self.Trigger_type.set(data["Triggerbot_Type"])
                    self.sensitivity_slider.set(data["Sensitivity"])

                    self.Smoothing_Title.configure(text=f"Smoothing - {int(self.Smoothing_Slider.get())}%") 
                    self.FOV_Title.configure(text=f"FOV - {int(self.FOV_Slider.get())} x {int(self.FOV_Slider.get())}")
                    self.Threshhold_Title.configure(text=f"Threshhold - {int(self.Threshhold_Slider.get())}")
                    self.sensitivity_title.configure(text=f"Sensitivity - {int(self.sensitivity_slider.get())}%")
        ...
        self.ConfigList = customtkinter.CTkOptionMenu(
            self.general_frame, values=os.listdir("configs"), width=200)
        self.ConfigList.place(x=20, y=20)

        self.ConfigInput = customtkinter.CTkEntry(self.general_frame, placeholder_text="Config name", width=160, font=(
            "Watchword Bold Demo", 12))
        self.ConfigInput.place(x=230, y=20)

        self.ConfigLoad = customtkinter.CTkButton(self.general_frame, text="Load", width=60, font=(
            "Watchword Bold Demo", 12), command=Load_config)
        self.ConfigLoad.place(x=20, y=60)

        self.ConfigSave = customtkinter.CTkButton(self.general_frame, text="Save", width=60, font=(
            "Watchword Bold Demo", 12), command=Save_config)
        self.ConfigSave.place(x=89, y=60)

        self.ConfigDelete = customtkinter.CTkButton(self.general_frame, text="Delete", width=60, font=(
            "Watchword Bold Demo", 12), command=Delete_config)
        self.ConfigDelete.place(x=158, y=60)

        self.ConfigCreate = customtkinter.CTkButton(self.general_frame, text="Create", width=160, font=(
            "Watchword Bold Demo", 12), command=Create_config)
        self.ConfigCreate.place(x=230, y=60)

        self.BhopToggle = customtkinter.CTkSwitch(self.general_frame, text="Bhop", font=(
            "Watchword Bold Demo", 14))
        self.BhopToggle.place(x=20, y=100)

        self.DebugToggle = customtkinter.CTkSwitch(
            self.general_frame, text="Debug mode (Beta)", font=("Watchword Bold Demo", 14))
